...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    Slice<Product> findActiveProductsAndIdLessThan(@Param("cursor") Long cursor, Pageable pageable);

    //상품 목록 조회 (삭제된 데이터)
    @Query("SELECT p FROM Product p WHERE p.isDeleted = true ORDER BY p.createdAt DESC")
    Slice<Product> findDeletedProducts(Pageable pageable);

    //Cursor 기준 이전 상품 목록 조회 (삭제된 데이터)
    @Query("SELECT p FROM Product p WHERE p.isDeleted = true AND p.id < :cursor ORDER BY p.createdAt DESC")
    Slice<Product> findDeletedProductsAndIdLessThan(@Param("cursor") Long cursor, Pageable pageable);
